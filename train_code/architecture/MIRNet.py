...
class MIRNet(nn.Module):
    def __init__(self, in_channels=3 * opt.frame, out_channels=opt.num_pixels * opt.num_wavelengths, n_feat=opt.mirnetfeats, kernel_size=3, stride=2, n_RRG=2, n_MSRB=1, height=3,
                 width=1, bias=False):
        super(MIRNet, self).__init__()

        self.conv_in = nn.Conv2d(in_channels, n_feat, kernel_size=kernel_size, padding=(kernel_size - 1) // 2,
                                 bias=bias)
        self.inp_proj = nn.Conv2d(3 * opt.frame, out_channels, kernel_size=1, bias=False)
