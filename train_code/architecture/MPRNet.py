...
class SAM(nn.Module):
    def __init__(self, n_feat, img_ch, kernel_size, bias):
        super(SAM, self).__init__()
        self.conv1 = conv(n_feat, n_feat, kernel_size, bias=bias)
        self.conv2 = conv(n_feat, img_ch, kernel_size, bias=bias)
        self.conv3 = conv(img_ch, n_feat, kernel_size, bias=bias)
...
class MPRNet(nn.Module):
    def __init__(self, in_c=24, out_c=opt.num_pixels * opt.num_wavelengths, n_feat=24, scale_unetfeats=48, scale_orsnetfeats=32, num_cab=4, kernel_size=3, reduction=1, bias=False):
        super(MPRNet, self).__init__()
        # in_c是网络“宽度”，并不是实际的输入通道

        self.conv_in = nn.Conv2d(3 * opt.frame, in_c, kernel_size=kernel_size, padding=(kernel_size - 1) // 2,
                                 bias=bias)
        self.inp_proj = nn.Conv2d(3 * opt.frame, out_c, kernel_size=1, bias=False)  # 新增：输入残差映射 6 → out_c(=225) 通道
        ...
        self.sam12 = SAM(n_feat, n_feat, kernel_size=1, bias=bias)
        self.sam23 = SAM(n_feat, n_feat, kernel_size=1, bias=bias)
    ...
    def forward(self, x3_img):
        ...
        x3_inp_pad = F.pad(x3_inp, [0, pad_w, 0, pad_h], mode='reflect')
        x3_inp_proj = self.inp_proj(x3_inp_pad)

        return (stage3_img + x3_inp_proj)[:, :, :h_inp, :w_inp]
