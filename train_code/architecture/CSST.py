...
class SS_MSA(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.only_local_branch:
            ...
            '''
            # 现在 out.shape == [b, nW, Ws*Ws, C]

            B, nW, num_tokens, C = out.shape  # nW = 49, num_tokens = 256
            W0, W1 = self.window_size  # (8, 8)
            # 实际做的是 2×window_size，所以
            B0, B1 = W0 * 2, W1 * 2  # 16, 16
            H, W = h, w  # 原输入高宽 112, 112
            nH, nW_ = H // B0, W // B1  # 7, 7   (窗口网格大小)

            # 把 49 个窗口还原成 7×7 的网格，再展开成 112×112
            out = rearrange(
                out,  # [B, 49, 256, C]
                'b (nh nw) (p0 p1) c -> b (nh p0) (nw p1) c',
                nh=nH, nw=nW_, p0=B0, p1=B1
            )  # [B, 112, 112, C]'''
            ...
            '''
            pos_emb2 = self.pos_emb2
            seq_len = sim2.shape[-1]

            if pos_emb2.shape[-1] != seq_len:
                # Use F.interpolate for resizing. It needs a 4D input (N, C, H, W).
                # Current shape: [1, 1, heads//2, L, L]. We squeeze and unsqueeze the head dimension.
                pos_emb2 = F.interpolate(
                    pos_emb2.squeeze(2),  # Shape -> [1, 1, L, L]
                    size=(seq_len, seq_len),
                    mode='bilinear',
                    align_corners=False
                ).unsqueeze(2) # Shape -> [1, 1, 1, seq_len, seq_len]
...
class SST(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x[:,:opt.num_wavelengths * opt.num_pixels,:,:]          #[b,28,256,256]
...
class QECNet(nn.Module):
    def __init__(self, in_nc=opt.num_wavelengths * opt.num_pixels, out_nc=5, channel=64):   #out_nc = num*2  --> out_nc = num
...
class CSST(nn.Module):
    def __init__(self, num_iterations=1):                       # num_iterations = 5
        ...
        self.para_estimator = QECNet(in_nc=opt.num_wavelengths * opt.num_pixels, out_nc=num_iterations)
        ...
        self.down1 = nn.Conv2d(3 * opt.frame, 3, 1, stride=1, padding=0)
        self.down2 = nn.Conv2d(3 * opt.frame, 3, 2, stride=2, padding=0)
        self.fution = nn.Conv2d(10, opt.num_wavelengths * opt.num_pixels, 1, padding=0, bias=True)
        self.num_iterations = num_iterations
        self.denoisers = nn.ModuleList([])
        for _ in range(num_iterations):
            self.denoisers.append(
                SST(in_dim=232, out_dim=opt.num_wavelengths * opt.num_pixels, dim=128, num_blocks=[1,1,1]),
            )
...
class Csst(nn.Module):
    """将 `CSST` 封装成 **HDNet 同款接口**，仅保留必须的输入形态。"""

    def __init__(self, num_iterations=5):
        super().__init__()
        self.core = CSST(num_iterations=num_iterations)
        # 这些属性在某些自动构图 / 日志里会用到
        self.in_ch = 3 * opt.frame
        self.out_ch = opt.num_pixels * opt.num_wavelengths

    # ---------------------------------------------------------------- forward
    def forward(self, x):                # pylint: disable=arguments-differ
        """支持两种调用方式：

        1. **单张张量**：`net(y_l)` – 直接把测量图送进来。
        2. **二元组**：`net((y_l, gt))` – 与当前 `TrainDataset` 输出一致。
        """
        if isinstance(x, (tuple, list)):
            y_l = x[0]                   # (bgr, hyper) -> 取 bgr 当作 y_l
        else:                            # 已经是张量
            y_l = x

        b, _, h, w = y_l.shape
        device, dtype = y_l.device, y_l.dtype

        # CSST 仍然需要 three‑input 形式 → 造“占位”
        y_h = torch.zeros(b, 4,  h // 2, w // 2, device=device, dtype=dtype)
        pattern = torch.zeros(b, 3 * opt.frame, h * 2, w * 2, device=device, dtype=dtype)


        return self.core(y_h, y_l, pattern)


# ---------------------------------------------------------------- factory ----

def make_csst(*, num_iterations=5) -> nn.Module:
    """与 `HDNet` 的工厂函数保持一致。"""
    return Csst(num_iterations=num_iterations)
