...
class SAHAB(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        '''
        if mask is not None:
            h = min(h, mask.shape[1])
            w = min(w, mask.shape[2])

        hh, ww = self.patch_size[0] * 2, self.patch_size[1] * 2  # (=16,16)
        h_trim = h - (h % hh)
        w_trim = w - (w % ww)
        if h_trim != h or w_trim != w:  # 只有不整除才裁剪
            x = x[:, :h_trim, :w_trim, :]
            if mask is not None:
                mask = mask[:, :h_trim, :w_trim]
            h, w = h_trim, w_trim  # 更新形状'''

        w_size = self.patch_size
        '''
        assert (mask is None) or (mask.shape[1] == h and mask.shape[2] == w), \
            f"mask size {mask.shape} ≠ x size ({h},{w})"'''
        ...
        x = rearrange(x, 'b (nh hh) (nw ww) c-> b (nh nw) (hh ww c)', hh=2, ww=2)
        mask = rearrange(mask, 'b (nh hh) (nw ww) -> b (nh nw) (hh ww)', hh=2, ww=2)
        N = x.shape[1]
        '''
        nh = h // (w_size[0] * 2)  # 重新算一次窗口行列数
        nw = w // (w_size[1] * 2)
        N = nh * nw'''
        ...
        if 0:
            ...
            # B, NumWin, token_dim = x_select.shape
            # x_select = x_select.reshape(B * NumWin, token_dim // c, c)
            x_select = x_select.reshape(b*N//2,-1,c)
            x_select = self.attn(x_select)+x_select
            # x_select = x_select.view(B, NumWin, token_dim)
            ...
            x = self.attn(x) + x
            x = x.view(b, N, -1)
        # x = rearrange(x, 'b (nh nw) (hh ww c) -> b (nh hh) (nw ww) c', nh=nh, hh=w_size[0] * 2, ww=w_size[1] * 2)
        # x = rearrange(x, 'b (nh nw) (hh ww c) -> b (nh hh) (nw ww) c', nh=h//(w_size[0] * 2), hh=w_size[0] * 2, ww=w_size[1] * 2)
        x = rearrange(x, 'b (nh nw) (hh ww c) -> b (nh hh) (nw ww) c', nh=2, hh=2, ww=2)
...
class SAHABs(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        for block in self.blocks:
            ...
            # 统一block输出：如果是元组，提取主张量
            if isinstance(block_output, tuple):
                x = block_output[0]  # 只取元组的第一个元素（主输出）
            else:
                x = block_output
...
class Sparsity_Estimator(nn.Module):
    def __init__(self, dim=opt.num_pixels * opt.num_wavelengths, expand=2, sparse=False):
        ...
        # Input projection
        self.in_proj = nn.Conv2d(dim, dim, 1, 1, 0, bias=False)
...
class CST(nn.Module):
    def __init__(self, out_ch=opt.num_pixels * opt.num_wavelengths, dim=opt.cstfeats, stage=2, num_blocks=[2, 2, 2], sparse=False):
        ...
        self.inp_proj = nn.Conv2d(3 * opt.frame, out_ch, kernel_size=1, bias=False)

        # Fution physical mask and shifted measurement
        self.fution = nn.Conv2d(3 * opt.frame + 3 * opt.frame, dim, 1, 1, 0, bias=False)

        # Sparsity Estimator
        if num_blocks==[2,4,6]:
            self.fe = nn.Sequential(Sparsity_Estimator(dim=dim,expand=2,sparse=False),
                        Sparsity_Estimator(dim=dim, expand=2, sparse=sparse))
        else:
            self.fe = Sparsity_Estimator(dim=dim, expand=2, sparse=sparse)
        ...
        # Output projection
        self.out_proj = nn.Conv2d(dim, out_ch, 3, 1, 1, bias=False)
    ...
    def forward(self, x, mask=None):
        ...
        if mask is None:
            mask = torch.zeros_like(x[:, :opt.frame*3, :, :])  # 只要 1 个通道即可
        # Fution
        ...
        if self.sparse:
            ...
            fea = self.fe(x)
            mask = torch.randn((b,1,h_inp,w_inp)).cuda()
        ...
        for (Blcok, FeaDownSample, MaskDownSample) in self.encoder_layers:
            fea = Blcok(fea, mask)
            '''
            h_c = min(fea.shape[-2], mask.shape[-2])
            w_c = min(fea.shape[-1], mask.shape[-1])
            if mask.shape[-2:] != (h_c, w_c):
                mask = mask[..., :h_c, :w_c]'''
            masks.append(mask)
            fea_encoder.append(fea)
        ...
        for i, (FeaUpSample, Blcok) in enumerate(self.decoder_layers):
            ...
            # 获取跳跃连接的特征图
            skip = fea_encoder[self.stage - 1 - i]

            # 1. 对齐空间尺寸
            if fea.shape[-2:] != skip.shape[-2:]:
                # 上采样编码器特征图以匹配解码器特征图尺寸
                skip = F.interpolate(
                    skip,
                    size=fea.shape[-2:],
                    mode='bilinear',
        ...
        # Output projection
        x_inp_pad = F.pad(x_inp, [0, pad_w, 0, pad_h], mode='reflect')
        x_inp_proj = self.inp_proj(x_inp_pad)
        out = self.out_proj(fea).view(x_inp_proj.shape) + x_inp_proj
        if self.sparse:
