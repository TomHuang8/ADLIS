...
class MST(nn.Module):
    def __init__(self, in_dim=28, out_dim=28, dim=28, stage=6, num_blocks=[2,4,4]):
...
class MST_Plus_Plus(nn.Module):
    def __init__(self, in_channels=3 * opt.frame, out_channels=opt.num_pixels * opt.num_wavelengths, n_feat=98, stage=6):
    ...
    def forward(self, x):
        ...
        res = self.skip(x)            # 6 → 225    对齐维度
        feat = self.conv_in(x)        # 6 → 64
        feat = self.body(feat)        # 64 → 64
        out = self.conv_out(feat)     # 64 → 225
        out = out + res               # 225 + 225
