...
class HINet(nn.Module):
    ...
    def __init__(self, in_chn=opt.num_pixels * opt.num_wavelengths, out_chn=opt.num_pixels * opt.num_wavelengths, wf=opt.num_pixels * opt.num_wavelengths, depth=4, relu_slope=0.2, hin_position_left=0, hin_position_right=4):
        super(HINet, self).__init__()

        self.conv_in = nn.Conv2d(3 * opt.frame, in_chn, kernel_size=3, padding=(3 - 1) // 2,
                                 bias=False)
        self.inp_proj = nn.Conv2d(3 * opt.frame, out_chn, kernel_size=1, bias=False)
        self.depth = depth
    ...
    def forward(self, x):
        ...
        # image_proj = self.inp_proj(image)
        # image_proj = self.conv_in(image_proj)
        out_2 = out_2 + image
