...
class BiSRNet_body(nn.Module):
    def __init__(self, in_dim=3*opt.frame, out_dim=324, dim=28, stage=2, num_blocks=[2,4,4]):
...
class BiSRNet(nn.Module):
    ...
    '''
    def __init__(self, in_channels=3*opt.frame, out_channels=opt.num_wavelengths*opt.num_pixels, n_feat=44, stage=3, num_blocks=[1,1,1]):
        ...
        self.fution = nn.Conv2d(12, 6, 1, padding=0, bias=True)                                            # 1-bit -> 32-bit
    ...
    def forward(self, y, Phi=None):
        ...
        """
        if Phi==None:
            Phi = torch.rand((y.shape[0],y.shape[1],y.shape[2],y.shape[3])).cuda()
